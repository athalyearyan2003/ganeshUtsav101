...
function MapNode({
  ...
  return (
    ...
    >
      ...
      <span className="whitespace-nowrap rounded-[6px] bg-canvas px-1.5 py-0.5 text-[13px] font-medium leading-[18px] text-ink">
        {label}
      </span>
    </div>
  );
...
export function MapDiagram({ nextName = 'Tulshibaug' }: { nextName?: string }) {
  ...
  return (
    <div className="relative h-full w-full overflow-hidden bg-sunken">
      <div key={recenterKey} className="absolute inset-0 animate-map-drift">
        <svg
          viewBox="0 0 390 520"
